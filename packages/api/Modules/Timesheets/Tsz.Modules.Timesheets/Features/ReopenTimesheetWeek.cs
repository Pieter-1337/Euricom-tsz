...
public sealed record ReopenTimesheetWeekCommand(Guid UserId, int IsoYear, int IsoWeek)
    : ICommand<TimesheetWeekDto>;

public sealed class ReopenTimesheetWeekValidator : AbstractValidator<ReopenTimesheetWeekCommand>
{
    public ReopenTimesheetWeekValidator(IUnitOfWork uow)
    {
        RuleFor(x => x)
            .MustAsync(async (cmd, ct) =>
            {
                ...
                return week.Status == TimesheetStatus.Submitted || week.Status == TimesheetStatus.Approved;
            })
            .WithError(TimesheetErrors.NotReopenable)
            .OverridePropertyName(string.Empty);
    }
}

public sealed class ReopenTimesheetWeekHandler(
    IUnitOfWork uow,
    IContractsAccessModule contracts,
    IWorkdaysAccessModule workdays)
    : ICommandHandler<ReopenTimesheetWeekCommand, TimesheetWeekDto>
{
    public async Task<TimesheetWeekDto> HandleAsync(ReopenTimesheetWeekCommand command, CancellationToken ct = default)
    {
