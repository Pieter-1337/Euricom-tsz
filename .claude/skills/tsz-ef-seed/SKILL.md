...
`Modules/<Feature>/Tsz.Modules.<Feature>/Seeding/<Feature>Seeder.cs`:

```csharp
using Tsz.Infrastructure.Abstractions;
using Tsz.Modules.<Feature>.Domain.<Aggregate>;

namespace Tsz.Modules.<Feature>.Seeding;
...
Register the seeder in the module's `<Feature>Module.RegisterServices()`:
...
public void RegisterServices(IServiceCollection services, IConfiguration configuration)
{
    ...
    services.AddScoped<<Feature>Seeder>();  // ADD THIS
}
```

In `Tsz.Api/Program.cs`, after `Migrate()`, resolve seeders from the startup scope:
...
using (var scope = app.Services.CreateScope())
{
    ...
    if (app.Environment.IsDevelopment())
    {
        ...
        var <feature>Seeder = scope.ServiceProvider.GetRequiredService<<Feature>Seeder>();
        await <feature>Seeder.SeedAsync();
    }
}
```
...
- Seeders are registered in the module's `RegisterServices()` so they're available in the DI container — the startup scope resolves them directly.
- Idempotency is critical: the seeder runs on every startup in development. Always guard with `ExistsAsync(...)` or a more specific check.
- One seeder per module; combine all seed data for that module into a single `SeedAsync()` method.
