...
Modify the entity in `packages/api/Modules/<Feature>/Tsz.Modules.<Feature>/Domain/<Aggregate>/<Aggregate>.cs` and/or its `<Aggregate>Configuration.cs`. Run `bun run check` to make sure it still compiles before generating the migration — `dotnet ef` builds the project, so a compile error blocks generation.
