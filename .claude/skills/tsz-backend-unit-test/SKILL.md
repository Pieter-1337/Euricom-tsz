...
- Import from the module namespace: `using Tsz.Modules.<Feature>.*`
...
public class <Operation><Feature>HandlerTests
{
    ...
    {
        var repo = new Mock<IRepository<<Aggregate>>>();
        var uow = new Mock<IUnitOfWork>();
        uow.Setup(u => u.RepositoryFor<<Aggregate>>()).Returns(repo.Object);
        ...
        repo.Verify(r => r.Add(It.IsAny<<Aggregate>>()), Times.Once);
...
public class <Operation><Feature>ValidatorTests
{
    ...
    public async Task BusinessRuleViolation_Fails()
    {
        ...
        repo.Setup(r => r.ExistsAsync(It.IsAny<Expression<Func<<Aggregate>, bool>>>(), It.IsAny<CancellationToken>()))
...
repo.Setup(r => r.FirstOrDefaultAsDtoAsync<<Aggregate>Dto>(
        It.IsAny<Expression<Func<<Aggregate>, bool>>>(),
...
Note: entity builders live in `Tsz.Api.Tests/Builders/<Aggregate>Builder.cs` and build instances from the module's domain namespace (e.g., `Tsz.Modules.Users.Domain.Users.User`).
