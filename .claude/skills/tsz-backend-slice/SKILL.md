...
## Conventions
- Slice file: `Modules/<Feature>/Tsz.Modules.<Feature>/Features/<Operation><Feature>.cs` — e.g. `CreateUser.cs`, `GetUserById.cs`
- Each file contains: command/query record, validator (writes only), handler — all in `Tsz.Modules.<Feature>.Features`
- **Cross-module queries:** If this query will be dispatched by other modules, place the query record in `Tsz.Modules.<Feature>.Contracts/Queries/` (namespace `Tsz.Modules.<Feature>.Contracts.Queries`) and place the handler in the impl's `CrossModule/` folder (namespace `Tsz.Modules.<Feature>.CrossModule`). The handler is auto-discovered and registered.
- **Internal-only slices:** Both query/command and handler live in impl's `Features/`.
- Response shape lives **in the slice** unless ≥2 slices share the *exact* same shape — only then promote to a module-level DTO (e.g. `UserDto.cs` in the module impl root)
...
**Query example — `Features/GetUserById.cs` (internal-only):**
...
**Cross-module query example — `Contracts/Queries/UserExistsQuery.cs` + `CrossModule/UserExistsQueryHandler.cs`:**

Query goes in Contracts/Queries:
```csharp
using Tsz.Infrastructure.Abstractions;
...
Handler goes in impl CrossModule:
```csharp
using Tsz.Infrastructure.Abstractions;
...
Other modules can now inject `I<Feature>AccessModule` and call `ExecuteQueryAsync(new UserExistsQuery(...))`.

**Shared module-level DTO (`UserDto.cs` in the impl root)** — opts in to EF projection:
...
In `Endpoints/<Feature>Endpoints.cs` inside `Map()`, inject the handler/dispatcher interface and call it directly. Validation errors automatically throw `ValidationException`, caught by the global handler.
