...
using Tsz.Modules.<Feature>.Features;
...
public class <Feature>EndpointsTests : IntegrationTestBase, IAsyncLifetime
{
    ...
    private Task Seed<Aggregate>Async(/* args with sensible defaults */) => WithUowAsync(async uow =>
    {
    ...
    [Fact]
    public async Task Get<Aggregates>_ReturnsOk()
    {
        await Seed<Aggregate>Async();

        var response = await Client.GetAsync("/api/<feature>s");

        response.EnsureSuccessStatusCode();
        var items = await response.Content.ReadFromJsonAsync<List<<Aggregate>Dto>>(Json);
        Assert.NotNull(items);
    }

    [Fact]
    public async Task Create<Aggregate>_ValidRequest_ReturnsCreated()
    {
        var command = new Create<Aggregate>Command(/* valid values */);

        var response = await Client.PostAsJsonAsync("/api/<feature>s", command, Json);

        Assert.Equal(HttpStatusCode.Created, response.StatusCode);
        var dto = await response.Content.ReadFromJsonAsync<<Aggregate>Dto>(Json);
        Assert.NotNull(dto);
    }

    [Fact]
    public async Task Create<Aggregate>_InvalidRequest_ReturnsBadRequest()
    {
        var command = new Create<Aggregate>Command(/* invalid values */);
    ...
    [Fact]
    public async Task Create<Aggregate>_BusinessRuleViolation_ReturnsConflict()
    {
        // Seed an aggregate that violates uniqueness
        await Seed<Aggregate>Async();

        var command = new Create<Aggregate>Command(/* values that conflict */);
    ...
    [Fact]
    public async Task Get<Aggregate>ById_NonExisting_ReturnsNotFound()
    {
...
For seeding nuance — entities with private setters need the static `Create(...)` factory, then optionally calling named mutators (e.g. `user.Rename(...)`) before `Add` + `SaveChangesAsync`. See `UserEndpointsTests.SeedUserAsync` for the live shape.
